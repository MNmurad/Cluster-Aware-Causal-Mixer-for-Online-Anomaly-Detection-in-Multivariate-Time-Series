...
def create_config(window_size, features, embedding_dim=55, backend="cuda"):
    return xLSTMBlockStackConfig(
        mlstm_block=mLSTMBlockConfig(
            mlstm=mLSTMLayerConfig(
                conv1d_kernel_size=8, qkv_proj_blocksize=5, num_heads=4, round_proj_up_dim_up=True,
...
class xLSTMModel(nn.Module):
    def __init__(self, args) -> None:
        ...
        feats = args.c_in
        lstm_embedding_dim = args.d_model
        device = args.slstm_backend
        ######
        ...
        cfg = create_config(window_size=window_size, features=feats, embedding_dim=lstm_embedding_dim, backend=device)
