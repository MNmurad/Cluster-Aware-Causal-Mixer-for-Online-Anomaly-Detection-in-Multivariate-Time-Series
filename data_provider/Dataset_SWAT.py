...
class Dataset_SWAT(Dataset):
    def __init__(self, root_path, flag = 'train', size = None, 
                 ...
                 timeenc = 0, freq = 'h', cols = None, entity_id = None, cluster = None, downsample = 1,
                 featureSimilarity = 1, **kwargs):
    ...
    def __read_data__(self):
        ...
        normal_data = np.load(os.path.join(self.root_path, 'train.npy')) # './data/SWAT/train.npy')
        attack_data_xy = np.load(os.path.join(self.root_path, 'test.npz')) # np.load('./data/SWAT/test.npz')
        attack_data = attack_data_xy['x']
        attack_label = attack_data_xy['y']
        normal_label = np.zeros(normal_data.shape[0])
       
        # normal_data = normal_data[int(normal_data.shape[0] * 0.25):]
       
        # selected_columns = ['Timestamp', 'FIT101', 'LIT101', 'MV101', 'P101', 'AIT201', 'AIT202',
        ...
        #        'AIT503', 'AIT504', 'FIT501', 'FIT502', 'FIT503', 'P501', 'PIT501',
        #        'PIT503', 'P602']
  
        '''
        Paper - "Nominality Score Conditioned Time Series Anomaly
        Detection by Point/Sequential Reconstruction" ignores AIT201, P201
        
        Paper - "SensitiveHUE: Multivariate Time Series Anomaly Detection by
        Enhancing the Sensitivity to Normal Patterns" ignores P201, LIT401
        '''

        # # # df_normal_data[['AIT201', 'P201']] = 0 # index: 4, 9
        # # # df_attack_data[['AIT201', 'P201']] = 0
        
        # df_normal_data[['P201', 'LIT401']] = 0 # index: 9, 27
        # df_attack_data[['P201', 'LIT401']] = 0
        
        normal_data[:, [10, 28]] = 0 # [10, 28]
        attack_data[:, [10, 28]] = 0
